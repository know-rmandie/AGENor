...
CREATE OR REPLACE FUNCTION suivi_etudes.tableau(annee text, dreal text, bop text, pilote text, titre text, avanc text)
...
--- définition des requêtes
	...
	sql_detail:= sql_annee || ' AND ' || sql_dreal || ' AND ' || sql_bop || ' AND ' || sql_pilote || ' AND ' || sql_titre ||' ';
        sql_having:= sql_annee || ' AND ' || sql_dreal || ' AND ' || sql_bop || ' AND ' || sql_pilote ||' ';
---
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION suivi_etudes.tableau(text, text, text, text, text, text)
  OWNER TO postgres;
