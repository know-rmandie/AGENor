  ...
  try {
    ...
    while ($row=$result->fetch(PDO::FETCH_ASSOC)) {
		...
		if (($dreal!=$row['dreal']) OR ($bop !=$row['bop']) OR ($ligne_budgetaire_lb !=$row[ligne_budgetaire_lb]) ) {
			...
			$ligne =$ligne.'<td colspan=3>'.$row['dreal'].'&nbsp;-&nbsp;'.$row['bop'].'&nbsp;-&nbsp;'.$Tab_LB[$row['ligne_budgetaire_lb']].'&nbsp;</td>';
			$ligne =$ligne.'<td colspan=3 align="left">'.$affiche_dotation.'</td>';
...
<html>
<head>
<title>Liste des études - COPIL Octobre 2013</title>
...
function gotopage(id) {
	...
	voir_valorisation('suivi_etudes_hn_maj.php?id_etude_hn='+id+'&prec='+prec+'&suiv='+suiv);
} ;
...
function voir_valorisation(lien) {
	w = window.open(lien,"fiche","menubar=yes, status=yes, scrollbars=yes, menubar=yes, width=800, height=400");
	w.focus();
}
