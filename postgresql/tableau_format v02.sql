-- Function: suivi_etudes.tableau_format(text, text, text, text, text, text, text, text)

-- DROP FUNCTION suivi_etudes.tableau_format(text, text, text, text, text, text, text, text);
...
ma_table_A := '	
	...
	from ' || mon_schema || '.' ||ma_table ||' left join ' || mon_schema || '.dotation
	ON
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION suivi_etudes.tableau_format(text, text, text, text, text, text, text, text)
  OWNER TO postgres;
