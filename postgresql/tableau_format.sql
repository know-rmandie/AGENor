-- Function: suivi_etudes.tableau_format(text, text, text, text, text, text, text)

-- DROP FUNCTION suivi_etudes.tableau_format(text, text, text, text, text, text, text);

CREATE OR REPLACE FUNCTION suivi_etudes.tableau_format(annee text, dreal text, bop text, service text, pilote text, titre text, avanc text)
...
DECLARE
	...
	ma_table_B text:='';
BEGIN
--- suivi_etudes.tableau_format(annee text, dreal text, bop text, service text, pilote text, titre text, avanc text)
...
select 	 *
	from suivi_etudes.tableau_format('2016', 'SDTMI', '', '', '', '', '')  
	as
	f00(
	 index text,
	 id_etude_hn int,
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION suivi_etudes.tableau_format(text, text, text, text, text, text, text)
  OWNER TO postgres;
